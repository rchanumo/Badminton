...
class BadmintonEnv(gym.Env):
    def __init__(self):
        ...
        #taken from internet
        self.max_shuttle_speed = 30
        self.min_shuttle_speed = 20
        #angle range
        self.min_angle = -np.pi
        self.max_angle = np.pi
        self.min_action_space_attr = np.array([self.min_shuttle_speed, np.pi/6, self.min_angle/6])
        self.max_action_space_attr = np.array([self.max_shuttle_speed, np.pi/3, self.max_angle/6])
        self.range_action_space_attr = np.array([high - low for high, low in zip(self.max_action_space_attr, self.min_action_space_attr)])
        self.action_space = spaces.Box(low=np.zeros(self.min_action_space_attr.shape), high=np.ones(self.max_action_space_attr.shape))
        ...
        self.observation_space = spaces.Box(low=0, high=20, shape=(3,), dtype=np.float32)
        self.shuttle_trajectory = None
        
        self.destination_point = [0, 0, 0, 0]
        self.p1_hit = False
        self.totalReward = 0

        #the type of weakness in player 2 that player 1 needs to find
        self.weakness = 1

        #saved observation state called before new rally
        self.saved_observation_state = None
            
    def new_rally(self):
        self.saved_observation_state = self.setObservationState()
        # self.render()
        #players come back to center court
        x = np.random.uniform(0, self.court_rect['x']/2)
        y = np.random.uniform(0, self.court_rect['y']/2)
        z = np.random.uniform(0.1, self.pcone_h-0.01) 
        self.p_pos = {'p1':{'x':6.7-x, 'y':y}, 'p2':{'x':10.05, 'y':3.05}}
        ...
        # self.chance = 'p1'
        self.shuttle_rel_pos = {'k':-x, 'z':z, 't':0}
        self.shuttle_rel_pos_prev = {'k':-x, 'z':z, 't':-1}
        self.shuttle_trajectory = None
        self.shuttle_attr = {'vi':0.1, 'thetai':0.1, 'cx':6.7, 
                             'cy':y, 'psi':0, 'k0':-x, 'z0':z}
        self.chance = 'p1'
        #self.make_frame()
    ...
    def shuttle_trajectory_wrt_t(self, vi, thetai, t):
        ...
        try:
            ...
            print(vi, thetai/np.pi, cos(thetai), t)
    ...
    '''
    def checkEpisodeOver(self, points_before_action):
        return (self.player_points['p1'] - points_before_action['p1']) + (self.player_points['p2'] - points_before_action['p2']) != 0

    def getReward(self, chance, points_before_action):
        # self.totalReward = 0
        ...
        self.totalReward = 0
        '''
        if p1 hits the suttle-
            and p1 gets a point: give reward 1
            if p1 looses a point: give reward -0.02
            else: give -0.002

        '''  
        if(self.p1_hit):
            if(self.player_points['p1'] - points_before_action['p1']):
                self.totalReward = 1
                self.p1_hit = False
            elif(self.player_points['p2'] - points_before_action['p2']):
    ...
    def convertActionListToAction(self, action_list):
        action = {
                  's_attr': { 'vi': action_list[0],
                          'thetai': action_list[1],
                          'psi': action_list[2]
                        },
                 }
        return action
    
    def justBeforeLandingPoint(self):
        """
    ...
    def getRandomVelocityTheta(self):
        velocity = random.randint(self.min_shuttle_speed, self.max_shuttle_speed-10)
        theta = random.uniform(np.pi/4, np.pi/3)
    ...
    def getShuttleAttributesForP2(self):
        if self.shuttle_within_range('p2'):
            ...
            # thetai_val = 0.99
            psi = None
            if self.p_pos['p2']['y'] <= 2.03:
                psi = random.uniform(-np.pi/10, 0)
            elif self.p_pos['p2']['y'] >= 4.07:
                psi = random.uniform(0, np.pi/10)
            else:           
                psi = random.uniform(-np.pi/10, np.pi/10)
            #psi = 0
    ...
    def getShuttleAttributesForP1(self):
        if self.shuttle_within_range('p1'):
            #return one of hard coded trajectories
            speed_of_return, thetai_val = self.getRandomVelocityTheta()#30, -0.01
            psi = None
            if self.p_pos['p1']['y'] <= 2.03:
                psi = random.uniform(0, np.pi/10)
            elif self.p_pos['p1']['y'] >= 4.07:
                psi = random.uniform(-np.pi/10, 0)
            else:
                psi = random.uniform(-np.pi/10, np.pi/10)
            psi = 0
    ...
    def getPlayerAttributesForP1(self):
        if self.chance == 'p2' or self.shuttle_within_range('p1'):
            ...
            self.getDestinationPoint('p1')
            # self.destination_point = self.justBeforeLandingPoint()
            closest_x, closest_y, closest_z, closest_time = self.destination_point
            ...
            if(closest_x - self.p_pos['p1']['x'] > 0):
                ...
                if(closest_y - self.p_pos['p1']['y'] > 0):
                    if abs(closest_x - self.p_pos['p1']['x']) == 0:
    ...
    def getP2Action(self):
        action = {}
        final_landing_points = self.justBeforeLandingPoint()
        if self.shuttle_rel_pos_prev['t'] != -1 and final_landing_points[0] >= 8.93 and final_landing_points[0] <= 11.16:# and final_landing_points[1] >= 4.07:
            self.getDestinationPoint('p2')
            action['p_attr'] = {'v': 0.001, 'alpha': 0.001}
            action['s_attr'] = {'vi': 0.1, 'thetai': -0.1, 'psi': 0}
            return action
        action['p_attr'] = self.getPlayerAttributesForP2()
        action['s_attr'] = self.getShuttleAttributesForP2()
        return action
    
    def getDestinationPoint(self, chance):
        if(self.shuttle_trajectory is None):
            if(self.shuttle_rel_pos_prev['t'] == -1):
                sx, sy, sz = self.get_shuttle_pos()
                self.destination_point =  [sx, sy, sz, 0.001]
                return  
            self.getShuttleTrajectory(chance)
            # print(self.shuttle_trajectory)
            shuttle_trajectory = self.shuttle_trajectory.copy()
            shuttle_trajectory = [elem for elem in shuttle_trajectory if elem[0]<self.pcone_h]
            if(chance == 'p1'):
                shuttle_trajectory = [elem for elem in shuttle_trajectory if elem[1]<self.court_rect['x']/2]
            else:
                shuttle_trajectory = [elem for elem in shuttle_trajectory if elem[1]>self.court_rect['x']/2]
            # print(shuttle_trajectory,'\n')
            distance_to_reach = [sqrt(pow(elem[1] - self.p_pos[chance]['x'], 2) + pow(elem[2] - self.p_pos[chance]['y'], 2)) 
            for elem in shuttle_trajectory]
            time_to_reach = [dist/self.max_player_speed for dist in distance_to_reach]
            shuttle_trajectory = [elem for (elem, t) in zip(shuttle_trajectory, time_to_reach) if elem[3]>t]
            shuttle_trajectory.sort()
            if(shuttle_trajectory == []):
                self.destination_point = [0,0,0,5]
                return
            # print('len of shuttle_trajectory', len(shuttle_trajectory))
            self.destination_point = [shuttle_trajectory[-1][1], shuttle_trajectory[-1][2], shuttle_trajectory[-1][0], shuttle_trajectory[-1][3]]
        # return self.destination_point


    def getShuttleTrajectory(self, chance):
        ...
        #     closest_x, closest_y, closest_z, closest_time = self.justBeforeLandingPoint()
        ...
        for i in range(0, int(5/self.frame_rate)):
            ...
            if(z == 0):
                break
            xval = (k*cos(self.shuttle_attr['psi']) + self.shuttle_attr['cx'])
            yval = (k*sin(self.shuttle_attr['psi']) + self.shuttle_attr['cy'])           
            self.shuttle_trajectory.append([z, xval, yval, i*self.frame_rate])
    ...
    def setObservationState(self):
        ...
        # self.getShuttleTrajectory(self.chance)
        #obs += (self.shuttle_trajectory)
        # obs.append(self.destination_point)
    ...
    def step(self, action):
        ...
        """
        # print(self.shuttle_trajectory is None)
        points_before_action = dict(self.player_points)
        #Take action. Need to define action_state, as p1 action is action_state[action]
        chance = self.chance    
        action = np.multiply(action, self.range_action_space_attr) + self.min_action_space_attr 
        p1_action_ref = self.getP1Action()
        p1_action = p1_action_ref.copy()
        p1_action['s_attr'] = self.convertActionListToAction(action)['s_attr']
        ...
        #    print("BEFORE " + str(points_before_action['p1']) + ":" + str(points_before_action['p2']) + " AFTER " + str(self.player_points['p1']) + ":" + str(self.player_points['p2']))
        #episode_over = self.checkEpisodeOver(points_before_action)

        #while(self.shuttle_rel_pos['t'] < end_point[3]):#-2*self.frame_rate):
        saved_observation_state_shuttle_in_reach_p2 = None
        reward = 0 
        while (not self.shuttle_within_range('p1')):
            #before reaching p2 -> can go out(-ve), hits net(-ve), hits same side(-ve)
            #while not self.shuttle_within_range('p2'):
                #detect change in score to give reward.
                #would mean 
            #chance would be of p2
            #after reaching p2 -> p2 cant hit(+ve), p2 hits out(-ve), p2 hits net(-ve), p2 hits same side(-ve)
            #chance is of p1
            #self.render()
            if self.shuttle_within_range('p2'):
                saved_observation_state_shuttle_in_reach_p2 = self.setObservationState()
                #self.render()
            points_before_action = dict(self.player_points)
            chance = self.chance
            ...
            p2_action = self.getP2Action()
            self.act(p1_action, p2_action)
            reward += self.getReward(chance, points_before_action)
        #while(self.shuttle_rel_pos['t'] < end_point[3]):#-2*self.frame_rate):
        # while (not self.shuttle_within_range('p2')) :
        #     p1_action = self.getP1Action()
        #     p2_action = self.getP2Action()
        #     self.act(p1_action, p2_action)
        obs = None
        if saved_observation_state_shuttle_in_reach_p2 is not None:
            obs = saved_observation_state_shuttle_in_reach_p2
        else:
            obs = self.saved_observation_state
        # #if episode_over:
        # #    print(self.player_points)
        # flag = False
        # if((self.shuttle_trajectory is  None) and self.chance == 'p2'):
        #     flag = True
        # if(flag):
        #     break
        episode_over = False
    ...
    def render(self, mode='human'):
        ...
        if self.use_pygame:
            ...
            x, y, z, _ = self.justBeforeLandingPoint()
            ...
            self.render_court.landing_point = [y*100 - 305, x*100 - 670, z*100]
            sx, sy, sz = self.get_shuttle_pos()
            ...
            self.render_court.printDestinationBase()
            #self.render_court.printLanding()
            self.render_court.printLandingBase()
