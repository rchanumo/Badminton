...
class BadmintonEnv(gym.Env):
    def __init__(self):
        ...
        self.min_action_space_attr = np.array([self.min_player_speed, self.min_angle, self.min_shuttle_speed, np.pi/6, self.min_angle/6])
        self.max_action_space_attr = np.array([self.max_player_speed, self.max_angle, self.max_shuttle_speed-5, np.pi/3, self.max_angle/6])
    ...
    def getReward(self, chance, points_before_action, p1_action, p1_action_ref):
        ...
        if(deviation_from_ref < 0.1):
            ...
            self.totalReward += exp(-dist_from_shuttle)
        else:
            self.totalReward += -(1 - exp(-(deviation_from_ref))) 
        # return self.totalReward
