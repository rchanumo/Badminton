...
#1. reduction action space to only movement direction (alpha)
#2. Descrete rewards
#3. Reduce observation space to only p1-pos and shuttle destination point
...
class BadmintonEnv(gym.Env):
    def __init__(self):
        ...
        self.min_action_space_attr = np.array([self.min_angle,])
        self.max_action_space_attr = np.array([self.max_angle,])
    ...
    def getReward(self, chance, points_before_action, p1_action, p1_action_ref):
        self.totalReward = 0
        ...
        '''
        if p1 chance:
            p2 gets point give -1 reward 
        '''
        # if(chance == 'p1'):
        #   if(self.player_points['p2'] - points_before_action['p2']):
        #       totalReward = -1
        #   return totalReward

        '''
        if p1 is moving towrds the shuttle:
            give rewrad based on how close he is to the shuttle and the direction of movement
        '''
        deviation_from_ref = abs(p1_action['p_attr']['alpha'] - p1_action_ref['p_attr']['alpha'])
        if(deviation_from_ref < 0.1):
            dist_from_shuttle =  sqrt(pow(self.destination_point[0] - self.p_pos['p1']['x'], 2) + pow(self.destination_point[1] - self.p_pos['p1']['y'], 2))
            self.totalReward += 0.3#exp(-dist_from_shuttle)
        else:
            self.totalReward += -0.01#*(1 - exp(-(deviation_from_ref))) 
        return self.totalReward
    ...
    def getRandomVelocityTheta(self):
        velocity = random.randint(self.min_shuttle_speed, self.max_shuttle_speed-5)
    ...
    def step(self, action):
        ...
        p1_action['s_attr'] = p1_action_ref['s_attr']
        # p1_action['p_attr']['v'] = p1_action_ref['p_attr']['v']
        p2_action = self.getP2Action()
        #print(self.chance)
        #print(p1_action)
        #print(p2_action)
        self.act(p1_action, p2_action)
        reward = self.getReward(chance, points_before_action, p1_action, p1_action_ref)
        #if reward != 0:
